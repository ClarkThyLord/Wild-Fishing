...
export(StepDirections) var step_direction := StepDirections.LEFT

export var step_pixel_size := 16

export var step_draw_range := 32
...
func update() -> void:
	...
	var start_step := clamp(int(step - step_draw_range), 0, INF)
